...
st.title("🛩️ VIP & Military Flight Tracker")
st.markdown("世界中の政府専用機や軍用機をリアルタイムでトラッキングするサイトです。(完全無料版プロトタイプ)")

# 政府専用機マスターデータ (Wikipedia参考 ＋ Hexコード補完)
VIP_DB = {
    # 既存データ
...
# -----------------------------------------
# 修正: バックグラウンドCSV記録＆空港予測ロジック (VS Codeを閉じても動く独立プロセス版)
# -----------------------------------------
# -----------------------------------------
# CSV記録は GitHub Actions (tracker.py) 側で実行されるため、UI描画のみを担当
# -----------------------------------------
...
for flight in raw_data:
    ...
    callsign = str(flight[1]).strip()
    time_position = flight[3]
    lon = flight[5]
    lat = flight[6]
    alt = flight[7]
    
    # マスターデータと照合して特定機体のみを抽出
    if icao24 in VIP_DB:
        info = VIP_DB[icao24]
        
        # 出発地・目的地 (無料のADS-B生データからは取得できないため N/A としています)
        dep_3, dep_4 = "N/A", "N/A"
        arr_3, arr_4 = "N/A", "N/A"
        
        # Unixタイムスタンプを日時に変換
        time_str = datetime.fromtimestamp(time_position).strftime('%Y-%m-%d %H:%M:%S') if time_position else "N/A"
...
df = pd.DataFrame(tracked_flights)
...
for idx, row in df.iterrows():
    if pd.notnull(row["緯度"]) and pd.notnull(row["経度"]):
        popup_text = f"<b>{row['所属国']} 政府専用機</b><br>レジ: {row['レジ番号']}<br>コールサイン: {row['コールサイン']}"
        folium.Marker(
...
# スマホ画面の幅に自動で合わせるように変更
st_folium(m, use_container_width=True, height=500, returned_objects=[])

st.subheader("📋 現在飛行中のVIP・政府専用機")
if not df.empty:
    # 画面には必要なカラムだけを表示する
    st.dataframe(df[["レジ番号", "所属国", "機種", "コールサイン", "出発地", "目的地", "時刻", "高度(m)"]])
else:
    st.info("現在、データベースに登録されている政府専用機は飛行していないか、トランスポンダがオフになっています。")

st.markdown("---")
st.markdown("※技術的注記: 無料のADS-BトラッキングAPI（OpenSky）の仕様上、空港名のデータは含まれません。本システムでは裏側で1分ごとに検知座標を記録し、初検知位置を出発予測、最終検知位置を到着予測としてCSVに保存しています。")

# -----------------------------------------
# 追加: CSV履歴データの確認セクション
# -----------------------------------------
st.subheader("📁 トラッキング予測履歴 (1分ごとの自動記録)")

# 手動更新ボタン（画面をリロードして最新のCSVを読み込むため）
if st.button("🔄 履歴データを最新に更新"):
    st.rerun()
...
if os.path.isfile(CSV_FILE):
    try:
        ...
        with open(CSV_FILE, "rb") as file:
            st.download_button(
                label="📥 予測データをCSVでダウンロード",
                data=file,
                file_name="vip_flight_history.csv",
                mime="text/csv",
            )
    except Exception as e:
        st.error(f"データの読み込みに失敗しました: {e}")
else:
    st.info("現在記録されたデータはありません。アプリを起動したままにすると、1分ごとに自動記録が開始されます。")